...
def main():
    ...
    if not(os.path.isdir('{}_npy'.format(folder))):
        os.mkdir('{}_npy'.format(folder))

    ouputDir = '{}_npy'.format(folder)
    ...
    for label in labels:
        ...
        directory = "{}_segment/{}".format(folder,label)
        wavs = os.listdir(directory)
        print("Start extraction of class : ",label)
        for wav in tqdm(wavs):
            wav_npy, ts =extract_audio_training(wav,directory)
            np.save(os.path.join(ouputDir,wav[:-4]+'.npy'),wav_npy)
            report_list.append((wav,ts,wav.split("_")[0]))

    # report =  pd.DataFrame(report_list,columns=['NAME','TIME_STEP','LABEL'])
    # report.to_csv('{}_time_step.csv'.format(folder),index=False)
